...
/**
 *  This function assign work to all threads equally as contiguous number of rows in the imagea
 * 
 * for example, in case of 2 threads, thread 1 works on bottom half, thread 2 on upper half
 */
...
/**
 * This function assign work to all threads equally but using ** Overlapping** contiguous 
 * rows for each thread
 * 
 *  For example, in case of 2 threads and 2 rows per section, the first thread will work 
 *  on rows 0,1 - 4,5 - 8,9 and the second thread will work on rows 2,3 - 6,7 - etc..
 * 
 */
void taskAssignmentOverlappingPartition(WorkerArgs *args, int numThreads,
    float x0, float y0, float x1, float y1,
    int width, int height,
    int maxIterations, int output[]) 
{
    ...
    int maxIterations, int output[])
{
    ...
    taskAssignmentOverlappingPartition(args, numThreads,
                        x0,  y0,  x1,  y1,
                        width,  height,
